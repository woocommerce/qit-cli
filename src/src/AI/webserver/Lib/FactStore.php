...
class FactStore {
	...
	public static function list(?int $step = null,
	                             ?string $kind = null,
	                             int $limit = 20): array {

		$matches = array_filter(self::$facts, function ($f) use ($step, $kind) {
			if ($step !== null && ($f['step'] ?? null) !== $step)   return false;
			if ($kind !== null && ($f['kind'] ?? null) !== $kind)   return false;
			return true;
		});
