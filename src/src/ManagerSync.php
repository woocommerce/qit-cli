...
class ManagerSync {
	...
	public function maybe_sync( bool $force_resync = false ): void {
		if ( $force_resync ) {
			$this->manager_backend->get_cache()->delete( $this->sync_cache_key );
		}

		$manager_sync = $this->manager_backend->get_cache()->get( $this->sync_cache_key );
		...
		$this->manager_backend->get_cache()->set( $this->sync_cache_key, $manager_sync, $expiration );
	}
	...
	public function enforce_latest_version(): void {
		...
		$latest_version      = $this->manager_backend->get_cache()->get_manager_sync_data( 'latest_cli_version' );
		$minimum_cli_version = $this->manager_backend->get_cache()->get_manager_sync_data( 'minimum_cli_version' );
