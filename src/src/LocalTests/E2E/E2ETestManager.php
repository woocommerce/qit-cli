...
class E2ETestManager {
	...
	public function run_tests( E2EEnvInfo $env_info, string $test_mode, bool $bootstrap_only, ?string $shard = null ): int {
		...
		foreach ( $env_info->tests as $test_info ) {
			...
			// must-use-plugin.php.
			if ( file_exists( $test_info['path_in_host'] . '/qit/mu-plugin.php' ) ) {
				...
				try {
					$this->docker->run_inside_docker( $env_info, [ 'bash', '-c', "mv {$test_info['path_in_php_container']}/qit/mu-plugin.php /var/www/html/wp-content/mu-plugins/qit-mu-$plugin_slug.php" ], $env_vars );
