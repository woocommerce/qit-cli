...
abstract class QITCommand extends Command {
	...
	protected function build_env_info(
		...
		$cli_env = $this->parse_env_vars( $input->getOption( 'env' ) ? $input->getOption( 'env' ) : array(), $input->getOption( 'env_file' ) ? $input->getOption( 'env_file' ) : array() );
		...
		// Initialize env_config with config section
		$env_config = array();
		...
		$seen_slugs     = array();
		foreach ( isset( $env_config['plugins'] ) ? $env_config['plugins'] : array() as $plugin ) {
		...
		// Merge env vars from parse_env_vars, CLI takes precedence
		$config_env        = isset( $env_config['env'] ) ? (array) $env_config['env'] : array();
		$env_config['env'] = array_merge( $config_env, $cli_env );
		...
		foreach ( $merged_options as $key => $value ) {
			...
			if ( $mapped_key === 'plugins' && ! empty( $value ) ) {
				$cli_plugins = array();
				$cli_values  = is_array( $value ) ? $value : array( $value );
				...
				$cli_values                = is_array( $value ) ? $value : array( $value );
				$config_values             = isset( $env_config[ $mapped_key ] ) ? (array) $env_config[ $mapped_key ] : array();
		...
		if ( ! empty( $woo ) ) {
			$woo_plugin = EnvironmentVersionResolver::resolve_woo( $woo, $env_config['plugins'] ? $env_config['plugins'] : array() );
		...
		$deps = App::make( 'QIT_CLI\Config\PluginDependencies' )->get_dependencies(
			$env_config['plugins'] ? $env_config['plugins'] : array(),
			$env_config['themes'] ? $env_config['themes'] : array(),
			$env_config['dependencies_mode'] ? $env_config['dependencies_mode'] : 'activate'
		);

		// Add dependencies, avoiding duplicates
		foreach ( $deps['plugin'] as $dep_plugin ) {
			$dep_slug = $dep_plugin->slug; // PluginDependencies returns Extension objects
		...
		if ( empty( $env_config['php_extensions'] ) ) {
			$env_config['php_extensions'] = array();
		}
		App::make( 'QIT_CLI\Config\PluginDependencies' )->maybe_add_php_extensions( $deps['php_extension'], $env_config['php_extensions'] );

		// Parse volumes
		$env_config['volumes'] = App::make( 'QIT_CLI\Environment\EnvVolumeParser' )->parse_volumes( $env_config['volumes'] ? $env_config['volumes'] : array() );
		...
		foreach ( $env_config as $key => &$value ) {
			switch ( $key ) {
				...
				case 'wp':
					...
					break;
				case 'woo_version':
					// $value = EnvironmentVersionResolver::resolve_woo($value, $env_config['plugins'] ?? array());
					break;
			}
		}
		...
		$env_info->docker_images           = isset( $env_config['docker_images'] ) ? $env_config['docker_images'] : array();
		...
		$env_info->playwright_config       = isset( $env_config['playwright_config'] ) ? $env_config['playwright_config'] : array();
		...
		$env_info->env                     = isset( $env_config['env'] ) ? $env_config['env'] : array();
		...
		if ( ! empty( $env_config['extension_set'] ) ) {
			$env_info = App::make( 'QIT_CLI\Environment\ExtensionSetResolver' )->resolve( $env_info, array( 'overrides' => array( 'extension_set' => $env_config['extension_set'] ) ) );
		}
