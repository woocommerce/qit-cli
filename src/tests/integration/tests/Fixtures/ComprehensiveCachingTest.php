...
class ComprehensiveCachingTest extends TestCase {
	...
	public function test_extension_caching(): void {
		...
		// Should download extensions on first run
		$this->assertStringContainsString( 'extension', strtolower( $output1 ),
			'First run should process extensions' );
		...
		// Should use cached extensions
		if ( strpos( $output2, 'Using cached' ) !== false || strpos( $output2, 'already' ) !== false ) {
			$this->assertStringContainsString( 'cached', strtolower( $output2 ),
				'Second run should use cached extensions' );
		}
		
		// Should not re-download
		$downloadCount1 = substr_count( strtolower( $output1 ), 'downloading' );
		$downloadCount2 = substr_count( strtolower( $output2 ), 'downloading' );
		
		$this->assertLessThanOrEqual( $downloadCount1, $downloadCount2,
			'Second run should not download more than first run' );
	}
