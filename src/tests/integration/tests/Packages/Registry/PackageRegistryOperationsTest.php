...
namespace QIT\IntegrationTests\Packages\Registry;
...
class PackageRegistryOperationsTest extends TestCase {
	...
	public function test_package_publish_with_subpackages(): void {
		...
		$packageName = 'woocommerce/qit-integration-test-subpkgs-' . $uniqueId;
		$manifestPath = $packageDir . '/qit-test.json';
		$manifest = json_decode( file_get_contents( $manifestPath ), true );
		$manifest['package'] = $packageName;
		
		// Make subpackage IDs unique to avoid collisions
		$newSubpackages = [];
		foreach ( $manifest['subpackages'] as $oldId => $subpkgConfig ) {
			// Extract the last part of the ID and add unique suffix
			$parts = explode( '/', $oldId );
			$baseName = end( $parts );
			$newId = 'woocommerce/' . $baseName . '-' . $uniqueId;
			$newSubpackages[ $newId ] = $subpkgConfig;
		}
