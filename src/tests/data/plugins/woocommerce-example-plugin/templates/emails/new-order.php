...
<h2>
	...
	?>
	<a class="link" href="<?php echo esc_url( $order_edit_url ); ?>"><?php printf( __( 'Order #%s', 'woocommerce-example-plugin' ), $order->get_order_number() ); ?></a> (<?php printf( '<time datetime="%s">%s</time>', $order_date_c, $order_date_text ); ?>)
</h2>


<?php
if ( 'bacs' !== ( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->payment_method : $order->get_payment_method() ) || apply_filters( 'woocommerce_example_plugin_show_bacs_info', false ) ) {
