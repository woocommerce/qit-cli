...
/**
 * WC_Example_Plugin class.
 */
class WC_Example_Plugin {
	...
	public function __construct() {
		...
		// Admin section
		if ( is_admin() ) {
			include_once( 'class-wc-example-plugin-admin.php' );
			$this->admin = new WC_Example_Plugin_Admin();
		}
		...
		add_filter( 'woocommerce_translations_updates_for_woocommerce_example_plugin', '__return_true' );
	}

	public function send_notification_hooks() {

		// Hook emails
		if ( apply_filters( 'woocommerce_example_plugin_multiple_statuses_trigger', true ) ) {
			if ( apply_filters( 'woocommerce_example_plugin_purchase_pending_to_processing', true ) ) {
				add_action( 'woocommerce_order_status_pending_to_processing', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_pending_to_completed', true ) ) {
				add_action( 'woocommerce_order_status_pending_to_completed', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_on-hold_to_processing', true ) ) {
				add_action( 'woocommerce_order_status_on-hold_to_processing', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_on-hold_to_completed', true ) ) {
				add_action( 'woocommerce_order_status_on-hold_to_completed', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_failed_to_processing', true ) ) {
				add_action( 'woocommerce_order_status_failed_to_processing', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_failed_to_completed', true ) ) {
			...
			if ( apply_filters( 'woocommerce_example_plugin_purchase_pending_to_partial-payment', true ) ) {
				add_action( 'woocommerce_order_status_pending_to_partial-payment', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_on-hold_to_partial-payment', true ) ) {
				add_action( 'woocommerce_order_status_on-hold_to_partial-payment', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_failed_to_partial-payment', true ) ) {
				add_action( 'woocommerce_order_status_failed_to_partial-payment', array( $this, 'new_order' ) );
			}

			if ( apply_filters( 'woocommerce_example_plugin_purchase_pending_to_on-hold', false ) ) {
	...
	/**
	 * get_plugin_for_order function.
	 *
	 * @access public
	 * @return void
	 */
	public function get_plugin_for_order( $order, $type = 'purchases' ) {
		global $wpdb;
		...
		// Get plugin which match
		$plugin = $wpdb->get_results( "
			SELECT * FROM {$wpdb->prefix}example_plugin
			LEFT JOIN {$wpdb->prefix}example_notification_triggers ON ( {$wpdb->prefix}example_plugin.notification_id = {$wpdb->prefix}example_notification_triggers.notification_id )
			WHERE (
				object_id = 0
				OR
			...
			GROUP BY {$wpdb->prefix}example_plugin.notification_id
		" );

		if ( $plugin ) {
			foreach ( $plugin as $key => $notification ) {
				 ...
				 $triggers = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}example_notification_triggers WHERE notification_id = %d", $notification->notification_id ) );
	...
	/**
	 * get_plugin_for_product function.
	 *
	 * @access public
	 * @return void
	 */
	public function get_plugin_for_product( $_product, $type = 'low_stock' ) {
		global $wpdb;
	...
	public function send( $id, $object, $is_plain_text, $to, $subject, $message, $notification ) {
		...
		$email->id = "example_notification_{$notification->notification_id}";
		$email->object = $object;
	...
	public function new_order( $order_id ) {
		...
		// Get plugin
		$plugin = $this->get_plugin_for_order( $order, 'purchases' );

		if ( $plugin ) {

			// Prepare email
			$blogname = wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
			$email_heading = __( 'New Customer Order', 'woocommerce-example-plugin' );

			$subject = apply_filters( 'woocommerce_email_subject_new_order', sprintf( __( '[%s] New Order (%s)', 'woocommerce-example-plugin' ), $blogname, $order->get_order_number() ), $order, null );
	...
	public function low_stock( $product ) {
		...
		// Get plugin
		$plugin = $this->get_plugin_for_product( $product, 'low_stock' );

		if ( $plugin ) {

			$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);

			$subject = apply_filters( 'woocommerce_email_subject_low_stock', sprintf( '[%s] %s', $blogname, __( 'Product low in stock', 'woocommerce-example-plugin' ) ), $product, null );
			...
			if ( ! empty( $variation_id ) )
				$title = sprintf(__('Variation #%s of %s', 'woocommerce-example-plugin' ), $variation_id, get_the_title( $product->get_id() ) ) . ' ' . $sku;
			else
				$title = sprintf(__('Product #%s - %s', 'woocommerce-example-plugin' ), $product->get_id(), get_the_title( $product->get_id() ) ) . ' ' . $sku;

			foreach ( $plugin as $notification ) {

				$message = sprintf( __( 'Hi %s,', 'woocommerce-example-plugin' ), $notification->recipient_name ) . "\n\n";

				$message .= $title . __('is low in stock.', 'woocommerce-example-plugin' ) . "\n\n";
	...
	public function out_of_stock( $product ) {
		...
		// Get plugin
		$plugin = $this->get_plugin_for_product( $product, 'out_of_stock' );

		if ( $plugin ) {

			$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);

			$subject = apply_filters( 'woocommerce_email_subject_no_stock', sprintf( '[%s] %s', $blogname, __( 'Product out of stock', 'woocommerce-example-plugin' ) ), $product, null );
			...
			if ( ! empty( $variation_id ) ) {
				...
				$title = sprintf( __('Product #%1$s - %2$s', 'woocommerce-example-plugin' ), $product->get_id(), get_the_title( $product->get_id() ) ) . ' ' . $sku;
			}
			...
			foreach ( $plugin as $notification ) {
				...
				$message .= $title . __('is out of stock.', 'woocommerce-example-plugin' ) . "\n\n";
	...
	public function backorder( $args ) {
		...
		// Get plugin
		$plugin = $this->get_plugin_for_product( $product, 'backorders' );

		if ( $plugin ) {

			$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);

			$subject = apply_filters( 'woocommerce_email_subject_backorder', sprintf( '[%s] %s', $blogname, __( 'Product Backorder', 'woocommerce-example-plugin' ) ), $product, null );
			...
			if ( ! empty( $variation_id ) ) {
				$title = sprintf( __( 'Variation #%1$s of %2$s', 'woocommerce-example-plugin' ), $variation_id, get_the_title( $product->get_id() ) ) . $sku;
			} else {
				$title = sprintf(__('Product #%s - %s', 'woocommerce-example-plugin' ), $product->get_id(), get_the_title( $product->get_id() ) ) . $sku;
			}
			...
			foreach ( $plugin as $notification ) {
				...
				$message .= sprintf(__('%s units of %s have been backordered in order #%s.', 'woocommerce-example-plugin' ), $quantity, $title, $order_id ) . "\n\n";
	...
	public function refund( $order_id, $is_partial = false ) {
		...
		// Get plugin.
		$plugin = $this->get_plugin_for_order( $order, 'refunds' );
		...
		if ( $plugin ) {
			...
			if ( $is_partial ) {
				$subject       = apply_filters( 'woocommerce_email_subject_refund', sprintf( __( '[%s] Partial Order Refund (%s)', 'woocommerce-example-plugin' ), $blogname, $order->get_order_number() ), $order, null );
				$email_heading = __( 'Customer Partial Refund', 'woocommerce-example-plugin' );
			} else {
				$subject       = apply_filters( 'woocommerce_email_subject_refund', sprintf( __( '[%s] Order Refund (%s)', 'woocommerce-example-plugin' ), $blogname, $order->get_order_number() ), $order, null );
				$email_heading = __( 'Customer Refund', 'woocommerce-example-plugin' );
			}
...
$GLOBALS['wc_example_plugin'] = new WC_Example_Plugin();
