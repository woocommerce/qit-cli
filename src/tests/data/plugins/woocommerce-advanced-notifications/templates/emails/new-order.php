...
<p><?php echo wp_kses_post( sprintf( __( 'Hi %s,', 'woocommerce-advanced-notifications' ), esc_html( $recipient_name ) ) ); ?></p>

<p><?php
/* translators: 1: first name 2: last name */
printf( esc_html__( 'You have received an order from %s %s:', 'woocommerce-advanced-notifications' ), esc_html( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->billing_first_name : $order->get_billing_first_name() ), esc_html( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->billing_last_name : $order->get_billing_last_name() ) );
?></p>
...
<h2>
	...
	?>
	<a class="link" href="<?php echo esc_url( $order_edit_url ); ?>"><?php printf( __( 'Order #%s', 'woocommerce-advanced-notifications' ), esc_html( $order->get_order_number() ) ); ?></a> (<?php printf( '<time datetime="%s">%s</time>', esc_html( $order_date_c ), esc_html( $order_date_text ) ); ?>)
</h2>
		...
		foreach ( $order->get_items() as $item_id => $item ) :
			...
			<tr>
				<td class="td" style="text-align:left; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; word-wrap:break-word;"><?php
					...
					// Product name
					echo wp_kses_post( apply_filters( 'woocommerce_order_product_title', $item['name'], $_product ) );
					...
					if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
						echo $item_meta->meta ? '<br/><small>' . esc_html( nl2br( $item_meta->display( true, true ) ) ) . '</small>' : '';
					} else {
	...
	<tfoot>
		...
		if ( $show_totals ) :
			...
			if ( $totals ) {
				...
				?>
				<tr>
					<th class="td" scope="col" colspan="2" style="font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; text-align:left;"><?php esc_html_e( 'Total', 'woocommerce-advanced-notifications' ); ?></th>
		...
		if ( $order->get_customer_note() ) {
			?><tr>
				<th class="td" scope="row" colspan="2" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Note:', 'woocommerce-advanced-notifications' ); ?></th>
